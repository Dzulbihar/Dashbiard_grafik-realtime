        ...
        name: 'Pendapatan Domestik',
          ...
          <?php
            $query = oci_parse($koneksi,
              "SELECT AGENT from (SELECT * from (SELECT  AGENT,sum(TOTAL_PENDAPATAN) AS TOTAL_PENDAPATAN from DASHBOARDGRAFIK.PROD_PEND_PERCUSTOMER where TAHUN=$tahun_ini AND BULAN<=$bulan and LOKASI='DOM' GROUP BY LOKASI,TAHUN,AGENT) order by TOTAL_PENDAPATAN desc) where rownum <=10");
            oci_execute($query);
            ...
            while(($row = oci_fetch_array($query, OCI_BOTH)) != false)
            {
              ...
              $query1 = oci_parse($koneksi,"SELECT TOTAL_PENDAPATAN from (SELECT * from (SELECT  AGENT,sum(TOTAL_PENDAPATAN) AS TOTAL_PENDAPATAN from DASHBOARDGRAFIK.PROD_PEND_PERCUSTOMER where TAHUN=$tahun_ini AND BULAN<=$bulan and LOKASI='DOM' GROUP BY LOKASI,TAHUN,AGENT ) order by TOTAL_PENDAPATAN desc ) where rownum <=10 and AGENT='$browsername'");
              oci_execute($query1);
