    ...
    series: [{
        ...
        data:
            ...
            <?php 
              $query2 = oci_parse($koneksi,     
                " 
                SELECT SUM(JML_TEUS) AS JML_TEUS, BULAN  FROM DASHBOARDGRAFIK.PROD_PEND_PERBULAN WHERE TAHUN=$tahun_lalu AND BULAN<=$bulan AND LOKASI='INT' GROUP BY BULAN ORDER BY BULAN ASC
                "
              );
              oci_execute($query2);
        ...
        data:
            ...
            <?php 
              $query2 = oci_parse($koneksi,     
                " 
                SELECT SUM(JML_TEUS) AS JML_TEUS, BULAN  FROM DASHBOARDGRAFIK.PROD_PEND_PERBULAN WHERE TAHUN=$tahun_ini AND BULAN<=$bulan AND LOKASI='INT' GROUP BY BULAN ORDER BY BULAN ASC
                "
              );
              oci_execute($query2);
              while(($arus = oci_fetch_array($query2, OCI_BOTH)) != false)
