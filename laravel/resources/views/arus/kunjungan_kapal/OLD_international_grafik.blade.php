       ...
       //   color: '#ff0a18',
          data:
            ...
            <?php 
              $query2 = oci_parse($koneksi,     
                " 
                SELECT TARGET_RKAP, BULAN FROM DASHBOARDGRAFIK.V_TARGET_RKAP_PERBULAN WHERE Tahun=$tahun_ini AND BULAN<=$bulan AND TYPE='INT' AND SATUAN='SHIPCALL' ORDER BY BULAN ASC
                "
              );
              oci_execute($query2);
              while(($arus = oci_fetch_array($query2, OCI_BOTH)) != false)
