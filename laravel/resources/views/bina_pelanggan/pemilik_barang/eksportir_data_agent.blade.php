      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header">
            <h3 class="card-title">
              <b> Real time produksi pendapatan Eksportir {{$agent}}</b>
            </h3>
            <p align="right">
                @empty ($start_date)
          ...
          <div class="card-body">
            ...
            <form action="" method="get">
              <div class="row g-3 align-items-center">
                ...
                <div class="col-auto">
                  ...
                  <a href="{{url('/bina_pelanggan_pemilik_barang_eksportir')}}/{{($agent)}}/{{($start_date)}}/{{($end_date)}}" class="btn btn-primary" type="submit">Reset</a>
                </div>
              </div>
            </form>
            ...
            <a href="{{url('excel_pemilik_barang_eksportir_agent')}}/{{($agent)}}/{{($start_date)}}/{{($end_date)}}" target="_blank" class="btn btn-default btn-sm">
              <i class="fa fa-download"></i>
              Download Excel
            </a>
            <a href="{{url('/bina_pelanggan_pemilik_barang_eksportir')}}" class="btn btn-default btn-sm"> <i class="fa fa-undo"></i> Back </a>
          </div>
        </div>
      </div>
    </div>
...
<script type="text/javascript">
  $(document).ready(function () {
    var table = $('.yajra-datatable').DataTable({
        ...
        serverSide: true,
        ajax: {
          url:  "{{url('bina_pelanggan_pemilik_barang_eksportir')}}/{{($agent)}}/{{($start_date)}}/{{($end_date)}}",
